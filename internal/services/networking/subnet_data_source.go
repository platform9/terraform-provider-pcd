...
func (d *subnetDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"dns_publish_fixed_ip": schema.BoolAttribute{Computed: true, MarkdownDescription: "The subnet's opt-in flag for publishing its fixed IPs to the network's DNS zone. `false` does not mean they are unpublished: a provider network that is not external publishes them without it. The `pcd_networking_subnet` resource and the DNS guide explain the rules."},
