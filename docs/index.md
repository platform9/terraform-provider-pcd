...
- `cloud` (String) Name of a `clouds.yaml` entry to source configuration from. Falls back to `OS_CLOUD`. The file is searched at `$OS_CLIENT_CONFIG_FILE`, `./clouds.yaml`, `~/.config/openstack/clouds.yaml`, then `/etc/openstack/clouds.yaml`. Explicit provider arguments and `OS_*` environment variables override values from the file.
