...
- The final fix wave followed round 2 of the Opus panel (17 findings, 11 confirmed) and a final review of the whole stack. On the first branch, `dns_publish_fixed_ip` carries the same upgrade caveat as `dns_domain`, clearing the network's `dns_domain` is no longer called the switch that always stops publishing (a port or floating IP with its own DNS domain still publishes), Neutron is said to publish a record only when it creates a port or a port's `dns_name` or `dns_domain` changes, and the subnet data source calls the flag an opt-in. On the second, `pools.yaml` always carries `also_notifies` and the pool's `description` so that clearing either reaches Designate, duplicate pool ids are refused, the PCD-9946 warning quotes an IPv6 master's compressed form when it fits, the guide gives the worker its own rndc key copy (owner `pf9`, mode 0400) instead of a world-readable one and says the SSH private key lands in Terraform state, and the registry example runs BIND on 5353 and says to restart the worker. On the third, Create waits for the role to converge before it writes `settings`, whether or not `wait_until_converged` is set, because the per-role endpoint can return a previous assignment's settings until the expansion resets them; the `settings` description gives the designate-mdns restart as seconds to a minute; and unit tests cover Read's drift report and a settings-only Update. A short lab check of this wave followed and passed, with one finding, below.
- Tier 3d (2026-09-19) checked the final fix wave (`lab-tier-3d-report.md`): PASS on all seven steps. PCD's Designate worker runs `rndc` through `sudo designate-rootwrap-helper` as root, not as the Designate user, so the guide's reason for the key's owner was wrong; the key only needs to be readable by root (fixed in the guide and the example). A `pcd_host_cluster_role` create with `settings` and no `wait_until_converged`, over stale settings, waited for convergence (46 s, most of it resmgr lagging the host) before writing them, and `designate-mdns` was already listening on the new address right after; the write held for three minutes with a clean plan. The `designate-mdns` restart's timing held with earlier tiers: 5 to 14 s (tier 4), 33 s (tier 3c), and immediate here — all within the `settings` description's "seconds to a minute."
