...
outright — the next apply breaks on storage instead. Every volume it creates
goes to `error`, and the image's web-download import fails because the store
cannot allocate a volume for it, ending the apply with `glance recorded a
failed import: image ... could not be imported into store(s) ...`. Enable the
...
If an apply already failed that way, it left volumes behind that never reached
Terraform state, and they have to go before the next run: the image's
`image-<id>` backing volume in the service project, and the `workload-data`
volume. The provider deletes the image whose import failed, so the listing is
only a confirmation — an image it could not delete is named in the error.

```shell
pcdctl image list                      # cirros should be gone
