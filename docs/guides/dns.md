...
2. **Install BIND9 and create the rndc key.** On Ubuntu the `bind9` package
   ...
   fresh key named `rndc-key`. Designate's worker runs `rndc` itself, as the
   Designate user (`pf9` on a PCD host), not as BIND's user. It reads its own
   copy of the key under `/etc/designate/`, owned by the Designate user and
   readable only by that user. The pool's `rndc_key_file` points at that copy,
   and both copies hold the same secret. The `pf9` user exists on a PCD host
   before the `dns` role is assigned. If Designate runs as another user on your
   host (the example's `designate_user`), substitute it for `pf9` below.
   ...
   sudo install -o pf9 -m 0400 /etc/bind/rndc.key /etc/designate/rndc.key
   ```
...
user differs, set `designate_user` and give that user the rndc key copy from
step 2 of "Prepare the DNS host"; if the file differs, set `designate_conf`.
...
The listen address is a setting of the host's `pf9-designate` role.
