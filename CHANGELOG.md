...
- **The Community Edition example's tenant networks boot.** Its blueprint used a VLAN underlay,
  ...
  label and no VLANs on the switch. The guide explains both underlays. For a region already
  running the example, `underlay_type` is an in-place update rather than a replacement, and the
  tenant networks that already exist as VLANs keep their segmentation — the change applies to
  networks created after it.
